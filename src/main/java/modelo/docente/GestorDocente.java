...
public class GestorDocente {
	...
	public EstadoOperacion guardarTodo(IDocente doc) {
		try{
			...
			table = "Docentes";
			campos = "`Legajo`, `TipoDocumento`, `NroDocumento`,  `Estado`";
			valores = docente.getLegajo() + ", " + docente.getPersona2().getTipoDocumento().getId() + ", "
	...
	private String agregarCargoDocente2(IDocenteg docente, ICargoDocente cd) {
	...
	private String agregarIncentivo2(IDocente docente, IIncentivo incentivo) {
		ManejoDatos md = new ManejoDatos();
