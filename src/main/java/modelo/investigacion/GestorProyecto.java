...
public class GestorProyecto {
    ...
    public EstadoOperacion nuevoProyecto(IProyecto proyecto) {
        try {
            ...
            String campos =
                "`Id`, `Nombre`, `Descripcion`, `Director`, `Fecha_Presentacion`, `Fecha_Aprobacion`,`Fecha_Inicio`,`Fecha_Fin`,`Programa`,`Estado`";
            String valores =
                "\'" + proyecto.getId() + "\', \'" + proyecto.getNombre()
                    + "\', \'" + proyecto.getDescripcion() + "\', "
                    + proyecto.getDirector().getLegajo() + ", \'"
                    + proyecto.getFechaPresentacion() + "\', \'"
            ...
            return e.isEstado()
                ? new EstadoOperacion(CodigoEstado.INSERT_OK,
                    "El Proyecto se creó correctamente")
                : new EstadoOperacion(CodigoEstado.INSERT_ERROR,
                    "No se pudo crear el Proyecto");
        } catch (Exception var6) {
            return new EstadoOperacion(CodigoEstado.INSERT_ERROR,
                "No se pudo crear el Proyecto");
        }
    }
    ...
    public EstadoOperacion modificarProyecto(IProyecto proyecto) {
        try {
            ...
            String campos =
                "`Id` = \'" + proyecto.getId() + "\', `Nombre` = \'"
                    + proyecto.getNombre() + "\', `Descripcion`= \'"
                    + proyecto.getDescripcion() + "\', `Director`= \'"
                    + proyecto.getDirector().getLegajo()
                    + "\', `Fecha_Presentacion` = \'"
                    + proyecto.getFechaPresentacion()
                    + "\', `Fecha_Aprobacion` =\'"
    ...
    public List<IProyecto> listarProyecto(IProyecto proyecto) {
        ...
        try {
            ...
            for (Hashtable<String, String> reg : res) {
                ...
                Docente profesor =
                    new Docente(null, Integer.parseInt(reg.get("Jefe")), null,
                        null, null, null, null);
                p.setDirector(profesor);
                String[] Fecha_Aprovacion =
                    reg.get("Fecha_Aprovacion").split("-");
                p.setFechaAprobacion(
                    LocalDate.of(Integer.parseInt(Fecha_Aprovacion[0]),
                        Integer.parseInt(Fecha_Aprovacion[1]),
                        Integer.parseInt(Fecha_Aprovacion[2])));
                String[] Fecha_Presentacion =
                    reg.get("Fecha_Presentacion").split("-");
                p.setFechaPresentacion(
                    LocalDate.of(Integer.parseInt(Fecha_Presentacion[0]),
                        Integer.parseInt(Fecha_Presentacion[1]),
                        Integer.parseInt(Fecha_Presentacion[2])));
                String[] Fecha_Inicio = reg.get("Fecha_Inicio").split("-");
                p.setFechaAprobacion(
                    LocalDate.of(Integer.parseInt(Fecha_Inicio[0]),
                        Integer.parseInt(Fecha_Inicio[1]),
                        Integer.parseInt(Fecha_Inicio[2])));
    ...
    public EstadoOperacion AgregarIntegrante(IProyecto proyecto, IIntegrante integrante) {
        try {
            ...
            return e.isEstado()
                ? new EstadoOperacion(CodigoEstado.INSERT_OK,
                    "El integrante se agregó correctamente")
                : new EstadoOperacion(CodigoEstado.INSERT_ERROR,
    ...
    public EstadoOperacion quitarIntegrante(IProyecto proyecto, IIntegrante integrante) {
        try {
            ...
            return new EstadoOperacion(CodigoEstado.DELETE_OK,
                "El integrante se eliminÃ³ correctamente");
    ...
    public EstadoOperacion agregarSubsidio(IProyecto proyecto, ISubsidio subsidio) {
        try {
            ...
            String campos =
                "`Proyecto`, `anio`, `disposicion`, `Monto`, `observaciones`";
            String valores =
                "\'" + proyecto.getId() + "\', \'" + subsidio.getFecha()
                    + "\', \'" + subsidio.getDisposicion() + "\', "
                    + subsidio.getMontoTotal() + ", \'"
                    + subsidio.getObservaciones() + "`";
            e.insertar(table, campos, valores);
            return e.isEstado()
                ? new EstadoOperacion(CodigoEstado.INSERT_OK,
                    "El subsidio se agregó correctamente")
                : new EstadoOperacion(CodigoEstado.INSERT_ERROR,
                    "No se pudo agregar el subsidio");
        } catch (Exception var7) {
            return new EstadoOperacion(CodigoEstado.INSERT_ERROR,
                "No se pudo crear el subsidio");
        }
    }

    public EstadoOperacion quitarSubsidio(IProyecto proyecto, ISubsidio subsidio) {
        ManejoDatos md = new ManejoDatos();
        md.delete("`Subcidio`", "IdNew_Table = " + proyecto.getId()
    ...
    public EstadoOperacion agregarProrroga(IProyecto proyecto, IProrroga prorroga) {
        try {
            ...
            String campos =
                "`proyecto`, `disposicion`, `fecha_incio`, `fecha_fin`";
            String valores =
                "\'" + proyecto.getId() + "\', \'" + prorroga.getDisposicion()
                    + "\', \'" + prorroga.getFechaInicio() + "\', "
                    + prorroga.getFechaFin() + "`";
            e.insertar(table, campos, valores);
            return e.isEstado()
                ? new EstadoOperacion(CodigoEstado.INSERT_OK,
                    "El prorroga se agregó correctamente")
                : new EstadoOperacion(CodigoEstado.INSERT_ERROR,
                    "No se pudo agregar el prorroga");
        } catch (Exception var7) {
            return new EstadoOperacion(CodigoEstado.INSERT_ERROR,
                "No se pudo crear el prorroga");
        }
    }

    public EstadoOperacion quitarProrroga(IProyecto proyecto, IProrroga prorroga) {
        ManejoDatos md = new ManejoDatos();
        md.delete("`prorroga`", "`proyecto` = " + proyecto.getId()
    ...
    private String armarCondicion(IProyecto proyecto) {
        ...
        if (proyecto != null) {
            ...
            if (proyecto.getFechaAprobacion() != null) {
                condicion +=
                    " `Fecha_Aprovacion` = "
                        + Date.valueOf(proyecto.getFechaAprobacion())
                            .toString()
                        + "'";
            }
            if (proyecto.getFechaInicio() != null) {
                condicion +=
                    " `Fecha_Inicio` = '" + Date
                        .valueOf(proyecto.getFechaInicio()).toString()
                        + "'";
            }
